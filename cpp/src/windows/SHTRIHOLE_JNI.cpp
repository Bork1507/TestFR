...
JNIEXPORT jint JNICALL Java_SHTRIHOLE_1JNI_nativeConnect (JNIEnv *jenv, jobject jobj, jint portNumber, jint baud){
   ...
   char* szProgID = (char*)"AddIn.Drvfr";
   ...
   hr = pUnk->QueryInterface( IID_IDrvFR, (LPVOID*)&pDrvFR );
   pUnk->Release();
   if ( FAILED( hr ))
   {
   ...
   long error = 0;
   error=pDrvFR->set_Password(30);
   error=pDrvFR->set_ComNumber(portNumber);

    switch (baud){
        case 2400:
            error=pDrvFR->set_BaudRate(0);
            break;
        case 4800:
            error=pDrvFR->set_BaudRate(1);
            break;
        case 9600:
            error=pDrvFR->set_BaudRate(2);
            break;
        case 19200:
            error=pDrvFR->set_BaudRate(3);
            break;
        case 38400:
            error=pDrvFR->set_BaudRate(4);
            break;
        case 57600:
            error=pDrvFR->set_BaudRate(5);
            break;
        case 115200:
            error=pDrvFR->set_BaudRate(6);
            break;
        default:
            error=pDrvFR->set_BaudRate(1);
            break;
    }
   //cout << "error set_BaudRate - " << error << endl;

   error=pDrvFR->Connect();
